...
async function createPixCharge({ amountReais, payerName, payerDocument, description }) {
  // webhook_url por transação: o webhook cadastrado no painel (evento
  // cashin) NÃO disparou em produção (0 deliveries em 2026-08-22 com
  // cobrança completed) — o partner v1 só notifica a URL passada aqui.
  const webhookUrl = process.env.SYNCPAY_WEBHOOK_SECRET
    ? `https://pandora-admin-site.vercel.app/api/public/payments/webhook?key=${process.env.SYNCPAY_WEBHOOK_SECRET}`
    : undefined;
